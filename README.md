...
- 前端体验
  - 多上传位（默认 5 个，可增加）、可排序。
  - **[NEW] 批量上传**：支持一次选择多个文件，自动按名排序填充；支持拖拽排序。
  - **[NEW] 图片预览**：上传图片显示缩略图，PDF 显示图标。
  - 上传前选定目标大小并显示对应最大页数；前端统计总页数并提示是否超限。
  - 中/英双语，默认中国大陆显示中文，其它地区显示英文；PC-only 访问提示。
  - 压缩进度条（不确定进度）、防重复提交、失败重试提示。
  - **[NEW] 流量统计**：集成 Google Analytics 4 (GA4)，实时监测用户与地理位置。
- 运维
...
- 部署
  ...
  - **Nginx 配置**：确保 `root` 指向 `frontend/dist` 目录，而非旧目录。
  - **前端构建**：生产环境使用 `sed` 替换 worker 路径以修复 PDF MIME 问题。
  - **后端权限**：确保 systemd 服务对 `app.db` 及 `storage/` 目录有读写权限。
...
更新历史（要点）
- 2025-11-29：**重大更新**
  - **国内支付落地**：新增“激活码 (Redeem Code)”系统，支持周/月/年卡兑换，配合面包多实现国内支付闭环。
  - **登录与设备管理**：完成后端设备数限制（拒绝登录 + 临时 Token），前端新增设备管理页 (`/device-management`)，允许用户手动踢出旧设备。
  - **前端优化**：首页增加“三步走”指引；GA4 埋点上传/下载动作；每日免费次数改为 2 次且每日重置。
  - **架构完善**：新增 `plans.py` 路由与 `RedeemCode` 模型；新增 `auth.py` 的复杂依赖注入。
  - **待办事项 (High Priority)**：
    - **下载权限对接**：必须修改 `download_file` 接口，使其识别用户 `plan_expires_at` 字段，从而放行付费用户（目前尚未连接，买了会员也无法下载）。
    - **Paddle 订阅**：对接全球支付。
    - **多地区策略**：前端根据 IP 显示不同价格/支付方式。
  - 修复 PDF Worker 加载 MIME 错误（通过 `sed` 替换与静态文件加载）。
  - 解决 Nginx 路径配置错误导致的缓存问题。
  - 解决后端 SQLite 只读权限问题（systemd 服务重启与 chmod）。
  - 上线 Google Analytics 4 (GA4) 统计。
  - **交互升级**：支持批量上传文件、自动填充插槽、图片预览缩略图、拖拽排序（Drag & Drop）。
  - 调整页面标题为 VisaFileCompress。
